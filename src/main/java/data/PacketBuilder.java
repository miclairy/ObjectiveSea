...
public abstract class PacketBuilder {
    ...
    protected byte[] createHeader(AC35StreamMessage type) {
        ...
        addFieldToByteArray(header, AC35StreamField.MESSAGE_TYPE, type.getValue());
        addFieldToByteArray(header, AC35StreamField.HEADER_TIMESTAMP, Instant.now().toEpochMilli());
        addFieldToByteArray(header, AC35StreamField.HEADER_SOURCE_ID, this.sourceID);
        if (type.getLength() != -1) {
            addFieldToByteArray(header, AC35StreamField.MESSAGE_LENGTH, type.getLength());
        }
        return header;
    }
